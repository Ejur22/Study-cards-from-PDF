...
const StudyItem = ({ filename, score, correctAnswers, totalQuestions, onDownload, onRepeatTest, canRepeat }) => {
  let correctCount = 0
  let totalCount = totalQuestions

  if (correctAnswers) {
    try {
      const parsed = typeof correctAnswers === 'string' ? JSON.parse(correctAnswers) : correctAnswers
      correctCount = parsed.correct || 0
      totalCount = parsed.total || totalQuestions
    } catch (e) {
      correctCount = 0
    }
  }
    ...
    <div className="study-item">
      <div className="study-item-content">
        <div className="study-item-info">
          <button className="filename-button" onClick={onDownload}>
            {filename}
          </button>
        </div>
        ...
        <div className="study-item-actions">
          <div className="score">
            ...
            <div className="score-details">
              <span className="score-percentage">{score ? `${score.toFixed(1)}%` : 'N/A'}</span>
              <span className="score-text">({correctCount}/{totalCount})</span>
            </div>
          </div>
          {canRepeat && (
